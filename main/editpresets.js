...
document.addEventListener("DOMContentLoaded", function() {
    ...
    let customTimesOrder = JSON.parse(localStorage.getItem("customTimesOrder")) || [];

    // Sort customTimes based on the customTimesOrder
    customTimes.sort((a, b) => {
        const indexA = customTimesOrder.indexOf(a.time);
        const indexB = customTimesOrder.indexOf(b.time);
    ...
    $(document).on("sortupdate", "#sortable", function () {
        ...
        const updatedOrder = $("#sortable .preset-check").map(function () {
            return $(this).attr("id");
        }).get();
